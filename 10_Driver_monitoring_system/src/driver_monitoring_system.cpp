    ...
    switch (input_source_map[input_source])
    {
        ...
        case 1:
        {
            ...
            if(status == 1)
            {
                for(i=0;i<number_of_cameras;i++)
                {
                gstreamer_pipeline.push_back("v4l2src device=" + device_paths[i] + " ! video/x-raw, width=" + std::to_string(IMAGE_WIDTH) + ", height=" + std::to_string(IMAGE_HEIGHT) + " ! videoconvert ! appsink -v");
                }
                /* Initialize waylad */
