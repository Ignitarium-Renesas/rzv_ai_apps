...
uint8_t PreRuntime::ReadAddrmapTxt(std::string addr_file)
{
    ...
    while(getline(ifs, str))
    {
        ...
        /*No need to check the Object files starts from 0x0 since address can be dynamically reallocated. */
    }
    /*check org_address, size for DRPAI_ADRCONV is not in DRP-AI Driver used area.*/
    if (!((DRPAI_DRV_USED_MAX < drpai_obj_info.drpai_address.data_in_addr)
        || DRPAI_DRV_USED_MIN > (drpai_obj_info.drpai_address.desc_drp_addr+drpai_obj_info.drpai_address.desc_drp_size))) 
    {
        std::cerr << "[ERROR] Directory name="<<drpai_obj_info.data_inout.directory_name<<":This Object files is not supported by DRP-AI Driver."<<std::endl;
        std::cerr << "        It uses 0x"<< std::setw(8)<<std::hex <<DRPAI_DRV_USED_MIN <<
             "~ 0x"<< std::setw(8)<<std::hex <<DRPAI_DRV_USED_MAX <<" range of DRP-AI memory area." << std::endl;
        ifs.close();
        return PRE_ERROR;
    }
    ifs.close();
...
* Description   : Loads a vector data to memory via DRP-AI Driver
...
* Description   : Loads a file contents to vector.
...
*               : org_addr = the start address before conversion.
* Return value  : start address for Pre-Runtime Object files
*                 INVALID_ADDR if user input is invalid
******************************************/
uint64_t PreRuntime::GetStartAddress(uint64_t addr, drpai_data_t drpai_data, uint64_t org_addr)
{
    uint64_t drpai_mem_addr_end = drpai_data.address+drpai_data.size - 1;

    /*No need to check the addr=INVALID_ADDR since it is already done in Load()*/
    ...
    if ((addr & LOWER_24BITS) != (org_addr & LOWER_24BITS))
    {
        std::cerr << "[ERROR] Lower 24 bits of Pre-Runtime Object files start address (org_address) and start_addr must be the same."<< std::endl;
        std::cerr << "        org_address = 0x" << std::setw(8) << std::hex << org_addr << std::endl;
        return INVALID_ADDR;
    }
    return addr;
}
...
/*****************************************
* Function Name : SetConvAddress
* Description   : Function to register physical address of input data.
*                 Calls DRP-AI Driver SET_ADRCONV.
* Arguments     : start_addr = physical address of input data buffer.
*               : drpai_data = DRP-AI memory area details.
* Return value  : 0 if succeeded
*                 not 0 otherwise
******************************************/
uint8_t PreRuntime::SetConvAddress(uint64_t start_addr,  drpai_data_t drpai_data)
{
    uint8_t ret = 0;
    uint64_t addr = 0;
    /*Size of DRP-AI Pre-processing Runtime object files*/
    drpai_obj_info.data_inout.object_files_size = drpai_obj_info.drpai_address.desc_drp_addr 
                                                    + drpai_obj_info.drpai_address.desc_drp_size 
                                                    - drpai_obj_info.drpai_address.data_in_addr;
    /*Size of DRP-AI memory area used (16MB aligned)*/
    this->Occupied_size = (drpai_obj_info.data_inout.object_files_size + LOWER_24BITS ) & LOWER_24BITS_MASK;

    if(start_addr == INVALID_ADDR)
    {
        /*start_addr is set at the end of DRP-AI memory area */
        addr = drpai_data.address + drpai_data.size - Occupied_size;
    }
    else
    {
        /* Check the user specified address is in the DRP-AI memory area*/
        addr = GetStartAddress(start_addr, drpai_data, drpai_obj_info.drpai_address.data_in_addr);
        if (INVALID_ADDR == addr)
        {
            std::cerr << "[ERROR] Invalid argument: start_addr = 0x" <<std::setw(8)<<std::hex <<start_addr << std::endl;
            return PRE_ERROR;
        }
    }

    /* Set the DRP-AI virtual start address to 0, indicating the actual 40-bit physical address. */
    errno = 0;
    drpai_adrconv_t drpai_adrconv;
    drpai_adrconv.conv_address = addr;
...
uint8_t PreRuntime::Load(const std::string pre_dir, uint64_t start_addr, uint8_t mode)
{
    ...
    uint64_t start_address = start_addr;
    uint64_t addr = 0;
    uint64_t size = 0;
    uint8_t i = 0;
    ...
    const std::string drpai_file_path[NUM_OBJ_FILE_V2H] =
    {
        "",                         /*DRPAI_INDEX_INPUT*/
        dir+"/drp_desc.bin",        /*DRPAI_INDEX_DRP_DESC*/
        dir+"/drp_config.mem",      /*DRPAI_INDEX_DRP_CFG*/
        dir+"/drp_param.bin",       /*DRPAI_INDEX_DRP_PARAM*/
        dir+"/aimac_desc.bin",      /*DRPAI_INDEX_AIMAC_DESC*/
        dir+"/weight.bin",          /*DRPAI_INDEX_WEIGHT*/
        "",                         /*DRPAI_INDEX_OUTPUT*/
        dir+"/aimac_param_cmd.bin", /*DRPAI_INDEX_AIMAC_PARAM_CMD*/
        dir+"/aimac_param_desc.bin",/*DRPAI_INDEX_AIMAC_PARAM_DESC*/
        dir+"/aimac_cmd.bin",       /*DRPAI_INDEX_AIMAC_CMD*/
    };
    ...
    /*Define the start address and 
      Run virtual to physical address conversion for Pre-processing Runtime Object files*/
    ret = SetConvAddress(start_address, drpai_data0);
    if ( PRE_SUCCESS < ret )
    {
        std::cerr << "[ERROR] Failed to run virtual to physicall address conversion."<< std::endl;
        return PRE_ERROR;
    }
    ...
    for (i = 1;i<NUM_OBJ_FILE_V2H; i++)
    {
        ...
        /*Add base address*/
        addr += drpai_obj_info.data_inout.start_address;
        if (INDEX_P == i)
        {
        ...
        {
            /* Do nothing for output data index */
        }
        else
        {
...
* Arguments     : filename = filename to be written to memory
...
*                 not 0 otherwise
******************************************/
uint8_t PreRuntime::LoadFileToMem(std::string filename, unsigned long from, unsigned long size)
{
    ...
    errno = 0;
    obj_fd = open(filename.c_str(), O_RDONLY);
    if (0 > obj_fd )
    {
        std::cerr << "[ERROR] Failed to open " << filename << ": errno=" << errno << std::endl;
    ...
    for (i = 0 ; i<(drpai_data.size/BUF_SIZE) ; i++)
    {
        ...
        if ( 0 > ret)
        {
            std::cerr << "[ERROR] Failed to read "<<filename<<" : errno=" << errno << std::endl;
            ret_load_data = PRE_ERROR;
            goto end;
        }
...
/*****************************************
* Function Name : SetInputAddress
* Description   : Function to register physical address of input data.
*                 Calls DRP-AI Driver SET_ADRCONV.
* Arguments     : in_addr = physical address of input data buffer.
* Return value  : 0 if succeeded
*                 not 0 otherwise
******************************************/
uint8_t PreRuntime::SetInputAddress(uint64_t in_addr)
{
    int ret = 0;
    drpai_adrconv_t drpai_adrconv;
    ...
    if (in_addr != internal_param_val.pre_in_addr && in_addr != INVALID_ADDR)
    {
        ...
        /*Note: org_address will be dummy address that does not overlap occupied area by Object files.*/
        drpai_adrconv.org_address  = drpai_obj_info.drpai_address.desc_drp_addr 
                                    + drpai_obj_info.drpai_address.desc_drp_size ; 
        /*Align the lower 24 bits of org_address to conv_address*/
        drpai_adrconv.org_address  = ((drpai_adrconv.org_address + LOWER_24BITS) & LOWER_24BITS_MASK)
                                    + (drpai_adrconv.conv_address & LOWER_24BITS);
        drpai_adrconv.size         = drpai_obj_info.drpai_address.data_in_size;
        drpai_adrconv.mode         = DRPAI_ADRCONV_MODE_ADD;
#ifdef DEBUG_LOG
        std::cout<<"[INFO] Input Data Address Conversion"<<std::endl;
        std::cout<<"       conv_address=0x"<<std::hex<<drpai_adrconv.conv_address<<std::endl;
        std::cout<<"       org_address =0x"<<std::hex<<drpai_adrconv.org_address<<std::endl;
        std::cout<<"       size        =0x"<<std::hex<<drpai_adrconv.size<<std::endl;
#endif
        ...
        if (0 != ret)
        {
            std::cerr << "[ERROR] Failed to run SET_ADRCONV for DRP-AI input image : errno=" <<  errno << std::endl;
            return PRE_ERROR;
        }
    }
...
{
    uint64_t input_address = param->pre_in_addr;
    uint8_t ret = 0;
    ...
    if(param->input_copy_enabled)
    {
        ...
        ret = SetInput((void *)input_address);
        if(PRE_SUCCESS != ret )
        {
            return PRE_ERROR;
        }
        ...
        std::cout << "[TIME] SetInput() Processing Time : " << std::fixed << std::setprecision(2)<< diff << " msec" << std::endl;
#endif
        input_address = drpai_obj_info.drpai_address.data_in_addr + drpai_obj_info.data_inout.start_address;
        internal_param_val.pre_in_addr = input_address;
    }
    else
    {
        ret = SetInputAddress(param->pre_in_addr);
        if(PRE_SUCCESS != ret )
        {
            return PRE_ERROR;
        }
    ...
    proc[DRPAI_INDEX_INPUT].address            = input_address;
    ...
    proc[DRPAI_INDEX_AIMAC_CMD].address        = drpai_obj_info.drpai_address.aimac_cmd_addr   + drpai_obj_info.data_inout.start_address;
    ...
    proc[DRPAI_INDEX_AIMAC_PARAM_CMD].address  = drpai_obj_info.drpai_address.aimac_param_cmd_addr   + drpai_obj_info.data_inout.start_address;
