...
void draw_bounding_box(void)
{
    ...
    for (i = 0; i < det.size(); i++)
    {
        ...
        Point textleft2(x_min,y_min-CLASS_LABEL_HEIGHT);
        Point textright2(x_min+CLASS_LABEL_WIDTH2,y_min);
        Point textleft3(x_min,y_min);
        rectangle(g_frame, topLeft, bottomRight, Scalar(0, 0, 0), BOX_THICKNESS);
        /*solid rectangle over class name */  
        if (det[i].c == 1) 
        {  
        rectangle(g_frame, topLeft, bottomRight, Scalar(0, 255, 255), BOX_THICKNESS);  
        rectangle(g_frame, textleft, textright, Scalar(0, 255, 255), -1);
        putText(g_frame, result_str, textleft3, FONT_HERSHEY_SIMPLEX, CHAR_SCALE_XS, Scalar(0, 0, 0), BOX_CHAR_THICKNESS);
        }
        else
        {   
        rectangle(g_frame, topLeft, bottomRight, Scalar(225, 46, 227), BOX_THICKNESS); 
        rectangle(g_frame, textleft2, textright2, Scalar(225, 46, 227), -1);
        putText(g_frame, result_str, textleft3, FONT_HERSHEY_SIMPLEX, CHAR_SCALE_XS, Scalar(0, 0, 0), BOX_CHAR_THICKNESS);
        }
    }
    return;
...
int main(int argc, char *argv[])
{
    ...
    std::cout << "Starting safety helmet vest Application" << std::endl;
