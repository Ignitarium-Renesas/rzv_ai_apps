...
/***********************************************************************************************************************
* File Name    : hand_gesture_recognition.cpp
* Version      : 1.1.0
* Description  : Sample Application for hand_gesture_recognition using yolov3.
***********************************************************************************************************************/
...
void draw_bounding_box(void)
{
    ...
    uint32_t x = HAND_COUNT_STR_X;
    uint32_t y = HAND_COUNT_STR_X;
    ...
    for (i = 0; i < det.size(); i++)
    {
        ...
        if(det[i].prob >= det[max_prob_index].prob)
        {
            max_prob_index = i;
        }
    }

    if (det[max_prob_index].prob == 0)
    {
        return;
    }
    result_cnt++;
    ...
    result_str = label_file_map[det[max_prob_index].c];
    GESTURE = result_str; 
    int32_t x_min = (int)det[max_prob_index].bbox.x - round((int)det[max_prob_index].bbox.w / 2.);
    int32_t y_min = (int)det[max_prob_index].bbox.y - round((int)det[max_prob_index].bbox.h / 2.);
    int32_t x_max = (int)det[max_prob_index].bbox.x + round((int)det[max_prob_index].bbox.w / 2.) - 1;
    int32_t y_max = (int)det[max_prob_index].bbox.y + round((int)det[max_prob_index].bbox.h / 2.) - 1;
...
/*****************************************
 * Function Name : Hand_Gesture_Recognition
 * Description   : Function to perform over all detection
 * Arguments     : -
 * Return value  : 0 if succeeded
 *               not 0 otherwise
 ******************************************/
int Hand_Gesture_Recognition()
{   
    /* Temp frame */
    Mat frame1;
